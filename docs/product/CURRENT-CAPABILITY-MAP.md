...
| Documentos        | Motor de plantillas estructuradas por tipo (código automático, datos + renderer) con referencias inteligentes `@` y formatos emitidos `//`, sobre el control documental (versión automática, flujo, distribución, copias) | Activo | `/dashboard/documents`                                 | `documents` / `document_versions` (`structured_content`) / `document_relations` / `document_code_counters` |
