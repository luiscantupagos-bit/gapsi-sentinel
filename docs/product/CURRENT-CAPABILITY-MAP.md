...
| Documentos        | Motor de plantillas estructuradas por tipo (código automático, datos + renderer normalizado) sobre el control documental (versión automática, flujo, distribución, copias) | Activo | `/dashboard/documents`                                 | `documents` / `document_versions` (`structured_content`) / `document_code_counters` |
