...
| Documentos        | Motor de plantillas estructuradas por tipo (código automático, datos + renderer) con referencias inteligentes `@` y formatos emitidos `//`, presentación normalizada (encabezado con marca del cliente, pie de confidencialidad, tema documental por organización, control de cambios) y biblioteca por áreas con listado maestro, sobre el control documental (versión automática, flujo, distribución, copias) | Activo | `/dashboard/documents`                                 | `documents` / `document_versions` (`structured_content`) / `document_relations` / `document_themes` / `document_code_counters` |
